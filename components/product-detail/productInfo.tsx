    ...
    <div className="flex flex-col">
      ...
      <div className="flex gap-4 mt-6 text-xs uppercase tracking-widest text-gray-500">
        <span className="border border-gray-300 px-3 py-1">
          {product.product_categories?.map((category) => category.translations?.[0]?.name).join(", ")}
        </span>
        <span className="border border-gray-300 px-3 py-1">
          {product.product_materials?.map((material) => material.translations?.[0]?.name).join(", ")}
        </span>
      </div>
      ...
      <div className="mt-12 flex flex-col sm:flex-row gap-4">
        <a
          href={`https://wa.me/62817085750446?text=Hello, I'm interested in ${product.translations?.[0]?.name}`}
          target="_blank"
